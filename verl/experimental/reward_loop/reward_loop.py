...
class RewardLoopWorker:
    ...
    async def compute_score_batch(self, data: DataProto) -> list[dict]:
        has_run_batch = hasattr(self.reward_manager, "run_batch")
        print(f"[DIAG] compute_score_batch: len(data)={len(data)}, "
              f"reward_manager_type={type(self.reward_manager).__name__}, "
              f"has_run_batch={has_run_batch}", flush=True)
        if has_run_batch:
            run_batch_fn = getattr(self.reward_manager, "run_batch")
            if callable(run_batch_fn):
                print(f"[DIAG] compute_score_batch: calling run_batch", flush=True)
                return await run_batch_fn(data)

        print(f"[DIAG] compute_score_batch: FALLBACK to per-item scoring", flush=True)
        tasks = []
...
class RewardLoopManager:
    ...
    def _chunk_by_query_group(self, data: DataProto, num_workers: int) -> tuple[list, list[list[int]]]:
        """Split *data* into chunks so that rollouts sharing a query stay together.

        This is necessary for the ``tournament`` reward manager which needs to
        see all rollouts for the same query in one batch for relative ranking.

        Returns
        -------
        chunks : list[DataProto]
            Query-group-aware chunks, one per worker.
        original_indices : list[list[int]]
            For each chunk, the indices into *data* that were included, so the
            caller can reassemble results in the original order.
        """
        n = len(data)

        # Determine query groups using uid (same uid => same query).
        if "uid" in data.non_tensor_batch:
            uids = data.non_tensor_batch["uid"]
        else:
            # Fall back: each item is its own group.
            uids = np.arange(n)

        # Group indices by uid, preserving order.
        ...
        # Distribute query groups to workers (round-robin by group).
        worker_indices: list[list[int]] = [[] for _ in range(num_workers)]
        for group_idx, indices in enumerate(uid_to_indices.values()):
            worker_id = group_idx % num_workers
            worker_indices[worker_id].extend(indices)

        # Build DataProto chunks from gathered indices.
        chunks = []
        for indices in worker_indices:
            if not indices:
                chunks.append(data[0:0])  # empty DataProto slice
            else:
                chunks.append(data[indices])
        return chunks, worker_indices

    def compute_rm_score(self, data: DataProto) -> DataProto:
        if self.reward_model_manager is not None:
            self.reward_model_manager.wake_up()

        is_tournament = self.config.reward.reward_manager.name == "tournament"

        if is_tournament:
            # Ensure rollouts for the same query are sent to the same worker.
            chunks, original_indices_per_chunk = self._chunk_by_query_group(
                data, len(self.reward_loop_workers)
            )
        else:
            chunks = data.chunk(len(self.reward_loop_workers))
            # Build trivial index mapping for uniform chunks.
            original_indices_per_chunk = []
            offset = 0
            for chunk in chunks:
                size = len(chunk)
                original_indices_per_chunk.append(list(range(offset, offset + size)))
                offset += size
        ...
        # Reassemble outputs in the original data order.
        outputs_ordered: list[dict | None] = [None] * len(data)
        for chunk_outputs, indices in zip(outputs, original_indices_per_chunk, strict=True):
            for local_idx, global_idx in enumerate(indices):
                outputs_ordered[global_idx] = chunk_outputs[local_idx]
        outputs_flat = outputs_ordered
        ...
        # Collect the union of all keys across items to handle heterogeneous
        # extra info dicts (e.g. tournament multi-rollout vs single-item fallback).
        reward_extra_keys_set: set[str] = set()
        for info in reward_extra_infos:
            reward_extra_keys_set.update(info.keys())
        reward_extra_keys = sorted(reward_extra_keys_set)
        non_tensor_batch = {}
