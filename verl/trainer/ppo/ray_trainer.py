...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        #
        # IMPORTANT: Tournament reward manager needs to see ALL rollouts for
        # the same query simultaneously (batch-level comparison).  Streaming
        # per-item reward in the agent loop would bypass the tournament logic
        # entirely, so we force-disable streaming reward when tournament mode
        # is active.  Reward will instead be computed at batch level in fit().
        is_tournament_reward = self.config.reward.reward_manager.name == "tournament"
        enable_agent_reward_loop = (
            (not self.use_rm or self.config.reward.reward_model.enable_resource_pool)
            and not is_tournament_reward
        )
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        with marked_timer("gen_max", timing_raw, color="purple"):
                            ...
                            rm_scores = None
                            _need_batch_reward = self.use_rm or self.config.reward.reward_manager.name == "tournament"
                            if _need_batch_reward and "rm_scores" not in batch.batch.keys():
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        # Tournament reward manager requires batch-level computation
                        # (reward is NOT computed per-item in agent loop for tournament).
                        _need_batch_reward = self.use_rm or self.config.reward.reward_manager.name == "tournament"
                        if _need_batch_reward and "rm_scores" not in batch.batch.keys():
                            batch_reward = self._compute_reward_colocate(batch)
                            batch = batch.union(batch_reward)
